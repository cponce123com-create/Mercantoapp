...
export default function CheckoutPage() {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <Loader size={32} className="animate-spin text-primary" aria-hidden="true" />
  ...
  if (!user) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <User size={64} className="text-muted-foreground/30 mb-6" aria-hidden="true" />
        ...
        <div className="flex gap-3 flex-wrap justify-center">
          <button
            ...
            className="px-6 py-2 bg-primary text-white rounded-xl font-bold hover:bg-primary/90 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            Iniciar sesión
          </button>
          <button
            onClick={() => setLocation("/registro")}
            className="px-6 py-2 border border-primary text-primary rounded-xl font-bold hover:bg-primary/5 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            Registrarse
          </button>
        </div>
  ...
  if (items.length === 0 && !isSuccess) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <ShoppingBag size={64} className="text-muted-foreground/30 mb-6" aria-hidden="true" />
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.shippingAddress.trim() && formData.deliveryMethod === "delivery") {
  ...
  if (isSuccess && successOrderId) {
    const deliveryMethodLabel = formData.deliveryMethod === "pickup" ? "Recojo en tienda" : "Envío a domicilio";
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center p-4 bg-background relative overflow-hidden">
        ...
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl border border-border max-w-md w-full text-center relative z-10 animate-in zoom-in-95 duration-500">
          <div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner" aria-hidden="true">
            <CheckCircle size={40} />
          </div>
          ...
          <button
            onClick={() => setLocation("/")}
            className="w-full py-4 bg-primary text-white font-bold rounded-xl hover:bg-primary/90 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            Volver al inicio
          </button>
        </div>
    ...
    <div className="bg-background min-h-screen py-8 pb-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <div className="mb-6 flex items-center gap-3">
          <Link href="/">
            <button className="p-2 bg-white rounded-full shadow-sm hover:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="Volver atrás">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
          <div className="md:col-span-3 space-y-6">
            ...
            <fieldset className="bg-white p-6 rounded-2xl border border-border shadow-sm">
              <legend className="text-lg font-bold mb-4 flex items-center gap-2">
                <User size={18} className="text-primary" aria-hidden="true" /> Datos de contacto
              </legend>
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="phone" className="block text-sm font-medium text-foreground mb-1.5">Teléfono o WhatsApp *</label>
                  <input
                    id="phone"
                    type="tel"
            ...
            <fieldset className="bg-white p-6 rounded-2xl border border-border shadow-sm">
              <legend className="text-lg font-bold mb-4 flex items-center gap-2">
                <MapPin size={18} className="text-primary" aria-hidden="true" /> Dirección de entrega
              </legend>
              <div className="space-y-4">
                <div>
                  <label htmlFor="address" className="block text-sm font-medium text-foreground mb-1.5">Dirección completa *</label>
                  <input
                    id="address"
                    type="text"
                ...
                <div>
                  <label htmlFor="notes" className="block text-sm font-medium text-foreground mb-1.5">Notas adicionales (opcional)</label>
                  <textarea
                    id="notes"
            ...
            <fieldset className="bg-white p-6 rounded-2xl border border-border shadow-sm">
              <legend className="text-lg font-bold mb-4 flex items-center gap-2">
                <Truck size={18} className="text-primary" aria-hidden="true" /> Opciones de entrega
              </legend>
              <div className="space-y-3">
                <label className={`flex items-start gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all ${formData.deliveryMethod === "pickup" ? "border-primary bg-primary/5" : "border-border hover:border-border/80"}`}>
                  ...
                  <div>
                    ...
                    <div className="mt-3 bg-white p-3 rounded-lg border border-border/50 text-sm flex gap-2 items-start">
                      <MapPin size={16} className="text-muted-foreground shrink-0 mt-0.5" aria-hidden="true" />
                ...
                <label className="flex items-start gap-4 p-4 border-2 border-border/50 bg-muted/20 rounded-xl cursor-not-allowed opacity-60">
                  <div className="mt-0.5">
                    <input type="radio" name="delivery" disabled className="w-4 h-4" aria-label="Envío a domicilio (próximamente)" />
                  </div>
            ...
            <fieldset className="bg-white p-6 rounded-2xl border border-border shadow-sm">
              <legend className="text-lg font-bold mb-4 flex items-center gap-2">
                <CreditCard size={18} className="text-primary" aria-hidden="true" /> Pago
              </legend>
            ...
            <div className="bg-white p-6 rounded-2xl border border-border shadow-sm sticky top-24">
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex justify-between items-start gap-4">
                    <div className="flex gap-3">
                      <div className="w-12 h-12 bg-muted/30 rounded-lg flex items-center justify-center text-xl shrink-0" aria-hidden="true">
                        {item.icon}
                      </div>
                      <div>
              ...
              <form onSubmit={handleConfirm}>
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-4 bg-primary text-white font-bold text-lg rounded-xl hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                >
                  {isLoading ? (
                    <>
